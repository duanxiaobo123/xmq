...
$(document).ready(function(){
    $("#info_status").change(function(event) {
        var info_status = $(this).val();
        if((info_status == 80) || (info_status == 90)){
            $.ajax({
                type:"GET",
                url:"<?php echo site_url('student/ajax_add_grade_so') ?>",
                dataType:"json",
                success:function(data){
                    var html = '';
                    html+='<div class="control-group ht">';
                        html+='<label class="control-label">协议单号 :</label>';
                        html+='<div class="controls">';
                            html+='<input type="text" class="span6" name="c_no" id="c_no" placeholder="" value="" />';
                        html+='</div>';
                    html+='</div>';
                    html+='<div class="control-group ht">';
                        html+='<label class="control-label">上传合同 :</label>';
                        html+='<div class="controls">';
                            html+='<input name="file" type="file" class="upload_input" accept="image/*,png,jpeg,jpg,gif"/>';                   
                        html+='</div>';
                    html+='</div>'; 
                    html+="<div class='control-group'>";
                        html+="<label class='control-label'>任课老师 :</label>";
                        html+="<div class='controls'>";
                            html+="<select class='span6 m-wrap' name='grade_teacher_id' id='teacher_id'>";
                                html+="<option value='0'>--选择任课老师--</option>";
                                for(k in data){
                                    html+="<option value="+data[k].id+">"+data[k].full_name+"</option>";
                                }
                            html+="</select>";
                        html+="</div>";
                    html+="</div>";
                    ...
                    $(".banji").remove();
                    $(".ht").remove();
                    $("#info_status").parent().parent().after(html);
                }
            })
        }else{
            $("#teacher_id").parent().parent().remove();
...
$("#info_visit").live('change',function(){
    ...
    $.ajax({ //一个Ajax过程
        ...
        success: function(json){//如果调用php成功
            var html = '';
            html+='<option value="0">--选择状态--</option>';
            for(i in json){
...
$("#teacher_id").live('change',function(){
    var teacher_id = $(this).val();
    $.ajax({ //一个Ajax过程
        type:"get", 
        cache:false,
        url :"<?php echo site_url('student/ajax_second_teacher_grade') ?>/"+teacher_id,
        dataType:'json',//从php返回的值以 JSON方式 解释
        data:'', //发给php的数据
        success: function(json){//如果调用php成功
            var html = '';
            html+='<div class="widget-content nopadding table_rolly banji">';
                html+='<table class="table table-striped table-bordered with-check table-hover">';
                    html+='<thead>';
                        html+='<tr>';
                            html+='<th></th>';
                            html+='<th nowrap>编号</th>';
                            html+='<th nowrap>班级名称</th>';
                            html+='<th nowrap>现有人数</th>';
                            html+='<th nowrap>任课老师</th>';
                            html+='<th nowrap>备注</th>';
                        html+='</tr>';
                    html+='</thead>';
                    html+='<tbody>'
                    for(var k in json.grade){
                        html+='<tr>';
                            html+='<td nowrap>';
                                html+='<label for="grade'+json.grade[k].id+'">';
                                    html+='<input type="radio" class="grade" name="grade" id="grade'+json.grade[k].id+'" value="'+json.grade[k].id+'" />';
                                html+='</label>';
                            html+='</td>';
                            html+='<td nowrap>'+json.grade[k].id+'</td>';
                            html+='<td nowrap>'+json.grade[k].grade_name+'</td>';
                            html+='<td nowrap>'+json.grade[k].now_number+'</td>';
                            html+='<td nowrap>'+json.grade[k].grade_teacher_name+'</td>';
                            html+='<td nowrap>'+json.grade[k].remark+'</td>';
                        html+='</tr>';
                    }
                    html+='</tbody>';
                html+='</table>';
            html+='</div>';
            html+='<div class="control-group banji">';
                html+='<label class="control-label">总课次 :</label>';
                html+='<div class="controls">';
                    html+='<div class="input-prepend"> <span class="add-on">#</span>';
                        html+='<input type="text" name="total_class_times" id="total_class_times"  class="span11" value="">';
                    html+='</div>';
                html+='</div>';
            html+='</div>';
            html+='<div class="control-group banji">';
                html+='<label class="control-label">费用合计 :</label>';
                html+='<div class="controls">';
                    html+='<div class="input-prepend"> <span class="add-on">￥</span>';
                        html+='<input type="text" name="total_fees" id="total_fees"  class="span11" value="0.00">';
                    html+='</div>';
                html+='</div>';
            html+='</div>';
            html+='<div class="control-group banji">';
                html+='<label class="control-label">实收金额 :</label>';
                html+='<div class="controls">';
                    html+='<div class="input-prepend"> <span class="add-on">￥</span>';
                        html+='<input type="text" name="shishou_fees" id="shishou_fees" class="span11" value="0.00">';
                        html+='<input type="hidden" name="qian_fees" id="qian_fees" value="">';
                    html+='</div>';
                html+='</div>';
            html+='</div>';
            html+='<div class="control-group banji">';
                html+='<label class="control-label">优惠金额 :</label>';
                html+='<div class="controls">';
                    html+='<div class="input-prepend"> <span class="add-on">￥</span>';
                        html+='<input type="text" name="youhui_fees" id="youhui_fees" placeholder="" class="span11" value="0.00">';
                    html+='</div>';
                html+='</div>';
            html+='</div>';
            html+='<div class="control-group banji">';
                html+='<label class="control-label">交费备注 :</label>';
                html+='<div class="controls">';
                    html+='<input type="text"  class="span6" name="fees_remark" placeholder="" value="" />';
                html+='</div>';
            html+='</div>';
            html+='<div class="control-group banji">';
                html+='<label class="control-label">经办人 :</label>';
                html+='<div class="controls">';
                    html+='<select class="span6 m-wrap" name="responsible_id">';
                        html+='<option value="0">--选择经办人--</option>';
                        for(i in json.adviser){
                            html+="<option value='"+json.adviser[i].id+"' >"+json.adviser[i].full_name+"</option>";
            ...
            html+='</div>';
            html+='<div class="control-group banji">';
                html+='<label class="control-label">交费类型 :</label>';
                html+='<div class="controls">';
                    html+='<select class="span6 m-wrap" name="fees_type" id="fees_type">';
                        html+='<option value="0">--选择交费类型--</option>';
                        for(i in json.fees_type){
                            html+="<option value='"+i+"' >"+json.fees_type[i]+"</option>";
                        }
                    html+='</select>';
                html+='</div>';
            html+='</div>';
            html+='<div class="control-group banji">';
                html+='<label class="control-label">交费方式 :</label>';
                html+='<div class="controls">';
                    html+='<select class="span6 m-wrap" name="pay_type" id="pay_type">';
                        html+='<option value="0">--选择交费方式--</option>';
                        for(i in json.pay_type){
                            html+="<option value='"+i+"' >"+json.pay_type[i]+"</option>";
                        }
                    html+='</select>';
                html+='</div>';
            html+='</div>';
            html+='<div class="control-group banji">';
                html+='<label class="control-label">体验老师 :</label>';
                html+='<div class="controls">';
                    html+='<select class="span6 m-wrap" id="tiyan_teacher_id" name="tiyan_teacher_id" >';
                        html+='<option value="0">--选择体验老师--</option>';
                        for(i in json.teacher){
                            html+="<option value='"+json.teacher[i].id+"' >"+json.teacher[i].full_name+"</option>";
                        }
                    html+='</select>';
                html+='</div>';
            html+='</div>';
            html+='<div class="control-group banji">';
                html+='<label class="control-label">交费渠道 :</label>';
                html+='<div class="controls">';
                    html+='<select class="span6 m-wrap" id="source_id" name="source_id">';
                        html+='<option value="0">--请选择渠道--</option>';
                        for(var i in json.source){
                            var line  = "├";//前导线
                            var layer = json.source[i].layer;
                            //填加前导线
                            for (var m=1; m < layer; m++) { 
                                line  += "──";
                            }
                            html+='<option value="'+json.source[i].id+'">'+line+json.source[i].name+'</option>';
                        }
                    html+='</select>';
                html+='</div>';
            html+='</div>';
            html+='<div class="clear banji"></div>';
            html+='<p class="banji"></p>';
            $(".banji").remove();
            $("#teacher_id").parent().parent().after(html);
        }
    });
});
</script>
